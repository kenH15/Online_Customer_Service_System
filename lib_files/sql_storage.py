...
try:
    ...
    class ResponseTable(Base):
        ...
        FAQ = Column(Integer, default=0)

        statement_text = Column(String(255), ForeignKey('StatementTable.text', ondelete='CASCADE', onupdate='CASCADE'))
...
class SQLStorageAdapter(StorageAdapter):
    ...
    def __init__(self, **kwargs):
        ...
        default_uri = "mysql+pymysql://root:root@localhost/robot_"

        database_name = self.kwargs.get("database", '00000')

        self.database_uri = default_uri + str(database_name)
        self.engine = create_engine(self.database_uri + '?charset=utf8&use_unicode=1', convert_unicode=True)
